...
function assignPositionA() { 
  ...
  if (isChoiceValid == true) {
    ...
    window.alert("Atencion, opcion invalida!");
  }
}
...
function findEmptyPosition() { //cuando se completan todos los numeros y simbolos, y es la que guarda cada valor ingresado en la posicion correspondiente
  if (positionA == undefined) {
    ...
    window.alert("Perdone, no hay mas espacios disponibles!");
  }
}
...
function endGame() { //Checks whether user is correct
  if (sumTotal == positionG) {
    ...
    window.alert("Mas suerte la proxima!");
  }
}
